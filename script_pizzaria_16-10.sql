...
select nome, count(pedidos.id) from cliente left join pedidos on clientes.id = pedidos.cliente_id where clientes.nome = 'Alexander Santos' group by clientes.id, nome;
select * from pedidos;

select count(*) as total_pedidos from pedidos inner join clientes on pedidos.cliente_id = clientes where clientes.nome = 'Alexandre Santos';
...
select nome, telefone, pedidos.id as pedido, data, valor from clientes right join pedidos on clientes.id = pedidos.cliente_id;
select *  from pedidos;
...
select clientes.id, nome , sum(valor) as valor_gasto from clientes inner join pedidos on pedidos.cliente_id = clientes.id group by clientes.id, clientes.nome;
...
select avg(valor) as valor_medio_pedidos from pedidos;
...
select nome, count(pedidos.id) as quantidade_de_pedidos from pedidos right join clientes on clientes.id = pedidos.cliente_id group by nome;
...
select valor from pedios inner join clientes on clientes.id = pedidos.cliente_id order by valor desc limit 1;
...
select max(valor) as maior, min(valor) as minimo from pedidos inner join clientes on pedidos.cliente_id = clientes.id where nome ='Laura Madureira';
...
select clientes.nome, sum(quantidade) as total_pizzas from intens_pedidos inner join pedidos on itens_pedido.pedido_id = pedidos.id inner join clientes on pedidos.clientes_id
= clientes,id group by nome;
-- 19. Qual o pedido mais barato, exibir o nome do cliente e o valor do pedido.

select nome, pedidos.valor from pedidos inner join clientes on clientes.id = pedidos.cliente_id order by valor  limit 1;
